# -*- coding: utf-8 -*-
"""
Created on Sun Mar  1 11:44:38 2020
...
def findMinDist(dist, collected):
    ...
    min_key = -1
    for key in dist:
        if dist[key] < min_dist and (not key in collected):
            min_dist = dist[key]
            min_key = key
    return min_key
...
def BFSFindPathArc(arc, car_record, dist):
    ...
    while not q.empty():
        ...
        if arc1.from_node in graph_around:
        ...
        if cost < min_cost:
            min_cost = cost
            min_arc_id = arc1.id
        if arc1.to_node in map_graph:
            for arc2 in map_graph[arc1.to_node]:
                if not arc2.id in visited:
...
def CalCover(arc, geo):
    ...
    if i == 0:
        ...
        projection = CalProjection(arc.geometry_list[i - 1], arc.geometry_list[i], geo)
    cover = round(DistanceActual(projection, arc.geometry_list[0]) / arc.len, 3)
    return cover
...
def main():
    for car_i in range(0, car_num):
        ...
        for car_j in range(len(car_i_start_end_index_list)):
            car_ij_start_index = car_i_start_end_index_list[car_j][0] # 第i辆车的第j段数据的第一个定位数据
            car_ij_end_index = car_i_start_end_index_list[car_j][1] # 第i辆车的j段数据的第最后一个定位数据，不包括该条记录
            ...
            if len(arc_path) == 1:
                for j in range(car_ij_start_index+1, car_ij_end_index):
                    ...
                    if car_record_objects[j].time == car_record_objects[j - 1].time:
                        continue
                    arc_cover = [[arc_path[0] , CalCover(arc_objects[arc_path[0] - 1] , car_record_objects[j].geo)]]
                    ...
                    # 输出
                    output_line = [str(car_i + 1), car_record_objects[j - 1].time, car_record_objects[j].time, geo_output]
                ...
                for j in range(car_ij_start_index+1, car_ij_end_index):
                    ...
                    # 计算投影点在路段上的位置
                    arc_path_output_cover[0][1] = CalCover(arc_objects[arc_path_output_cover[0][0] - 1] , car_record_objects[j].geo)
...
map_file = open('C:/Users/86189/Desktop/地图匹配算法1/data11/map6.txt')
...
car_file = open('C:/Users/86189/Desktop/地图匹配算法1/data11/100cars.txt')
...
map_graph = {}
for arc in arc_objects:
    if arc.from_node in map_graph:
        map_graph[arc_objects[arc.id - 1].from_node].append(arc)
    else:
        map_graph[arc_objects[arc.id - 1].from_node] = [arc]

# 输出文件名字
result_file_name = 'C:/Users/86189/Desktop/地图匹配算法1/result11/output.txt'
